...
const SpecialOffer = () => {
  ...
  const displayList = Array.isArray(items)
    ...
    : [];
  return (
    <div className="bg-gradient-to-r from-[#1a1212] to-[#2a1e1e] text-white py-16 px-4 font-[Poppins]">
      <div className="max-w-7xl mx-auto ">
        <div className="text-center mb-14">
          <h1
            className="text-5xl font-bold mb-4 transform transition-all bg-gradient-to-r from-amber-400 to-red-500 
   ...
   // const cartItem = cartItems.find((ci) => ci.item._id === item._id);
            ...
            return (
              <div
                key={item._id}
                className="relative group bg-[#4b3b3b] rounded-3xl overflow-hidden shadow-2xl transform hover:-translate-y-4 transition-all duration-500
                hover:shadow-red-900/40 border-2 border-transparent hover:border-amber-500/20 before:absolute before:inset-0 hover:before:opacity-20"
              >
        ...
        <div className=" mt-12 flex justify-center">
          <button
            ...
            text-white px-8 py-4 rounded-2xl font-bold text-xl uppercase tracking-wider hover:gap-4 hover:scale-105
