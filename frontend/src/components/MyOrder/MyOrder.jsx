      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Failed to load orders. Please try again lateer"
        );
      } finally {
        setLoading(false);
      }
...
if (loading) {
  ...
  if (error)
    return (
      <div
        className=" min-h-screen bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d] flex items-center justify-center text-red-300
      text-xl gap-4"
      >
        <p>{error}</p>
        <button
                    ...
                    >
                      ...
                      <td className=" p-4">
                        <div className=" flex items-center gap-2">
                          ...
                          <span
                            className={`px-4 py-2 rounded-lg ${status.bg} ${status.color}
