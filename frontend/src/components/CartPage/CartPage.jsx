...
const CartPage = () => {
  ...
  return (
    <div
      className=" min-h-screen overflow-hidden py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-[#1a120b] via-[#2a1e14]
     to-[#3e2b1d]"
    >
      <div className="max-w-7xl mx-auto">
        ...
        {cartItems.length === 0 ? (
          <div className=" text-center animate-fade-in">
            ...
            <Link
              to="/menu"
              className=" transition-all duration-300 text-amber-400 inline-flex items-center gap-2
              hover:gap-3 hover:bg-amber-800/50 bg-amber-900/40 px-6 py-2 rounded-full font-cinzel text-sm uppercase"
            >
              Browser All Items
            </Link>
          </div>
...
{cartItems.map(({ _id, item, quantity }) => {
  if (!item) return null; // 👈 null check to prevent crash

  return (
    <div
      key={_id}
      className=" group bg-amber-900/20 p-4 rounded-2xl border-4 border-dashed border-amber-500
     backdrop-blur-sm flex flex-col items-center gap-4 transition-all duration-300
     hover:border-solid hover:shadow-xl hover:shadow-amber-900/10
    ...
    >
      ...
      <div className=" flex items-center gap-3">
        <button
          ...
          className=" w-8 h-8 rounded-lg bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 
      ...
      <div className=" flex items-center justify-between w-full">
        <button
          ...
          className=" bg-amber-900/80 px-3 py-1 rounded-full font-cinzel text-xs uppercase transition-all
         duration-300 hover:bg-amber-800/50 flex items-center gap-1
         active:scale-95"
        >
      ...
      {selectedImage && (
        <div
         ...
         bg-amber-900/40 bg-opacity-75 brackdrop-blur-sm p-4 overflow-auto"
        ...
        >
          <div className=" relative max-w-full max-h-full">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className=" absolute top-1 right-1 bg-amber-900/80 rounded-full p-2 text-black hover:bg-amber-800/90 transition-transform 
